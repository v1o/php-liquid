			...
			if($start_regexp->match($token))
			{
				if($tag_regexp->match($token))
				{
					...
					else
					{
						...
						// This will create the class name for tags that contain underscore and have camelcase naming convention.
						// Say for example you have a tag name "use_layout" you want to look for the class name Use_Layout not Use_layout.
						$tag_name_alternate = 'LiquidTag' . str_replace(' ', '_', ucwords(str_replace('_', ' ', $tag_regexp->matches[1])));
					}
					...
					else if ($tag_name_alternate && class_exists($tag_name_alternate))
					{
						$this->_nodelist[] = new $tag_name_alternate($tag_regexp->matches[2], $tokens, $this->file_system);
					}
					else
					{
